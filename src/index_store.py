...
def load_index(
...
) -> Tuple[Optional[np.ndarray], Optional[List[str]], Dict[str, object], bool]:
    files = _index_files(index_dir, backend.name)
    if not files["embeddings_norm"].exists() or not files["paths"].exists():
        return None, None, {"status": "missing"}, True
    ...
    if not signature_ok:
        return None, None, {"status": "signature_mismatch", "meta": meta}, True
    ...
    if not stored_set.issubset(current_set):
        return None, None, {"status": "paths_mismatch", "meta": meta}, True

    embeddings_norm = np.load(files["embeddings_norm"])
    return embeddings_norm, stored_paths, meta, False
...
def build_index(
    ...
    embeddings_norm = normalize_embeddings(embeddings_arr).astype(np.float32)

    np.save(files["embeddings"], embeddings_arr)
    np.save(files["embeddings_norm"], embeddings_norm)
